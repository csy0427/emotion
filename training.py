...
class Trainer:
    ...
    def training(self, model_number, iteration_number):
        sentences = []
        with codecs.open(''.join([dirname(__file__), '/res/model', model_number]), 'r',
                         encoding='utf8') as f:  # /res/model# 파일 읽음
            for line in f:
        ...
        true_set = np.ones(
            [temp_set.shape[0] - 1, temp_set.shape[1], temp_set.shape[2] + 1])  # 사용자가 입력한 번호의 파일은 참값들이 모인 집합
        ...
        sentences = []
        for file_name in filter(lambda x: x != 'model' + model_number,
                                listdir(''.join([dirname(__file__), '/res']))):  # 사용자가 입력한 번호 외의 파일들 읽음
        ...
        false_set = np.zeros(
            [temp_set.shape[0] - 1, temp_set.shape[1], temp_set.shape[2] + 1])  # 번호 외의 파일들은 거짓값들이 모인 집합
